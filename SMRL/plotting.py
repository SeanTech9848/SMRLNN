...
def plot_evaluation_cont(results, configs, output_dir, data_train_path, data_test_path, exp_dirs_result_summary, filters=None):
    ...
    with open('%s/configs_sorted%s.txt' % (exp_dirs_result_summary, filter_str), 'w') as f:
        f.write('\n'.join(labels_long))

    ## YS
    ''' Compute evaluation summary and store'''
    eval_str = evaluation_summary(results_all, labels, output_dir, binary=False)
    # eval_str = evaluation_summary(results_all, labels, output_dir+'/'+exp_dirs_result_summary, binary=False)
    ...
    with open('%s/results_summary%s.txt' % (exp_dirs_result_summary, filter_str), 'w') as f:
