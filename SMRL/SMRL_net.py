...
from SMRL.util import *

class SMRL_net(object):
    ...
    def _build_graph(self, x, t, y_ , p_t, z_norm, FLAGS, r_lambda, r_beta, do_in, do_out, dims):
        ...
        smrlcca = -(  tf.reduce_mean(tf.log(2.)-tf.nn.softplus(-T_xy)) - tf.reduce_mean(tf.nn.softplus(-T_x_y) + T_x_y - tf.log(2.))  )
        ...
        # gmi_neg_loss2 = structure_keeper(x, h_rep_norm, k_top_cca)

        ## test
        # gmi_neg_loss = tf.reduce_sum(tf.diag_part(gmi_neg_loss2))
        # import pdb; pdb.set_trace()
        ...
        #     mxy = tf.matmul(tf.transpose(mean_x), mean_y) * tf.cast(tf.shape(x)[0], tf.float32)
        #     vx = tf.matmul(tf.transpose(x), y)
        #     cov_xy = vx - mxy
        #     return cov_xy
        #
        ...
        # # y_np = y.transpose()
        #
        # x_np = matx.eval(session=tf.Session()).transpose()
        # y_np = maty.eval(session=tf.Session()).transpose()
        #
        #
        # # ### for pass
        # # x_np = tf.transpose(matx)
        # # y_np = tf.transpose(maty)
        #
        # # x_np = matx.eval(session=tf.eval()).transpose()
        # # y_np = maty.eval(session=tf.eval()).transpose()
        #
        # xytol = np.vstack((x_np, y_np))
        # # xycov = np.cov(xytol, bias=False)
        # xycov = np.cov(xytol, bias=True)
        #
        #
        # ### for pass
        # px = x_np.shape[0]
        ...
        # syx_np = xycov[px:, :px]
        #
        # sxx_inv_np = np.linalg.inv(sxx_np)
        # syy_inv_np = np.linalg.inv(syy_np)
        #
        # Na = np.matmul(np.matmul(sxx_inv_np, sxy_np), np.matmul(syy_inv_np, syx_np))
        # Nb = np.matmul(np.matmul(syy_inv_np, syx_np), np.matmul(sxx_inv_np, sxy_np))
        #
        # eigenValues_a, eigenVectors_a = np.linalg.eig(Na)
        # eigenValues_b, eigenVectors_b = np.linalg.eig(Nb)
        #
        # idx_a = eigenValues_a.argsort()[::-1][:k_top_cca]
        # idx_b = eigenValues_b.argsort()[::-1][:k_top_cca]
        #
        # a = eigenVectors_a[:, idx_a]
        # b = eigenVectors_b[:, idx_b]
        #
        #
        # ## tf begin
        ...
        #
        # tf_xc = tf.matmul(matx, tf_a)
        # tf_yc = tf.matmul(maty, tf_b)
        #
        ...
        # #     my = tf.reduce_mean(y, axis=1)
        # #     xm, ym = x - mx, y - my
        # #     # t1_norm = tf.nn.l2_normalize(xm, axis = 1)
        # #     # t2_norm = tf.nn.l2_normalize(ym, axis = 1)
        # #     t1_norm = tf.nn.l2_normalize(xm, dim=1)
        # #     t2_norm = tf.nn.l2_normalize(ym, dim=1)
        # #     cosine = tf.losses.cosine_distance(t1_norm, t2_norm, dim=1)
        # #     return cosine
        #
        # # pearson_r(tf_xc, tf_yc)
        # # u = tf.gather(um, indices=idx, axis=1)
        #
        #
        # def pearson_ri(y_true, y_pred):
        #     # y_true = xci
        #     # y_pred = yci
        #     x = y_true
        #     y = y_pred
        ...
        #     # t1_norm = tf.nn.l2_normalize(xm, axis = 1)
        #     # t2_norm = tf.nn.l2_normalize(ym, axis = 1)
        #     t1_norm = tf.nn.l2_normalize(xm, dim=0)
        #     t2_norm = tf.nn.l2_normalize(ym, dim=0)
        #     cosine = tf.losses.cosine_distance(t1_norm, t2_norm, dim=0)
        #     return cosine
        #
        #
        # cca_loss = tf.constant(0, dtype=tf.float32)
        #
        # for i in range(k_top_cca):
        #     xci = tf.gather(tf_xc, indices=i, axis=1)
        #     yci = tf.gather(tf_yc, indices=i, axis=1)
        #     cca_lossi = pearson_ri(xci, yci)
        #     cca_loss = cca_loss + cca_lossi
        #
        # gmi_neg_loss = cca_loss
        #
        #
        #
        ...
        #without sigmoid
        ls2 = -tf.reduce_mean(d0) + tf.reduce_mean(d1) + r_beta*dp
        ...
        self.gmi_neg_loss = smrlcca
        self.discriminator_loss = ls2
