...
tf.app.flags.DEFINE_string('outdir', '../results/ihdplinshi/', """Output directory. """)
...
def run(outdir):
    ...
    ABNet = abnet.SMRL_net(x, t, y_, p, znorm, FLAGS, r_lambda, r_beta, do_in, do_out, dims)
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    import pytz
    timestamp = datetime.datetime.now(pytz.timezone('US/Eastern')).strftime("%m%d_%H:%M:%S")

    outdir = FLAGS.outdir+'/results_'+timestamp+'/'
    # outdir = FLAGS.outdir+'/results_/'
    os.mkdir(outdir)
